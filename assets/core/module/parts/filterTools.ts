...
function setThirdBranch(line: CountIndences) {
  return {
    ...line,
    third:
      typeof store?.branches?.third === "string"
...
function treeFormatter(this: any, line: CountIndences) {
  ...
  const isParentFolder =
    second === BRANCH_SECOND_CHILD || directoryName.match(/\/$/g);
  return `<div
        class="parsed-data">
        ${whitespaceWithVertical}${first}${second}${third}${"&nbsp;".repeat(
    store?.style?.offset || 0
  )}
